...
## AI 数据站 / GEO 变现?——已建完,产出 €0,17/90d;GEO 是期权不是渠道(2026-09-09,owner:「内容消费是永恒的主题…构建一个ai数据站是否价值最大?ai最终导流geo,如何突破geo实现创造营收?」)

**裁定全文 `docs/geo-revenue-research-2026-09.md`(三轮 prompt、四段链路逐段第一方读数、
事实表全带日期与可信度)。「内容消费永恒」成立,但推不出「所以建 AI 数据站」——
舰队三个月前就建完了(llms.txt / MCP / 逐页 markdown / CC BY 数据集 / for-agents),
本轮第一次量出它的产出。**

**四段链路的第一方读数(D1 现查 2026-09-09,后续会话直接引用,别再重测)**:
①**被抓** bpj 28 天 1 067 次引用爬取(ClaudeBot 339 / ChatGPT-User 324 / GPTBot 161 /
OAI-SearchBot 123 / PerplexityBot 120)→ ②**被引用** agi 564 次/30 天、判定题份额
33–37,5%(Bing,2026-08-16 截图)→ ③**送回真人** 各站 19–34 pv/28 天,占全站真人
**1,6–3,1%** → ④**转化** eco 90 天:**AI 引荐 6,1%(33 pv / 2 点击),搜索引荐 21,8%
(339 pv / 74 点击)**。**这条渠道 90 天的全部收入 = 2 × €0,085 = €0,17。**

**三条固定判断,后续会话别再推翻**:
1. **②→③ 断在结构上**,是答案引擎的 zero-click 产品设计,不是站点缺陷。
   「多写判定页 → 更多引用 → 更多流量 → 更多收入」这条因果链在第三个箭头上断裂,
   **任何 GEO 手法都改不动它**。
2. **③→④ 比基线还差**:行业普遍说「AI 流量转化是自然搜索 4–5 倍」(全为厂商自述),
   **本舰队实测方向相反**。再提「AI 流量更值钱」必须先拿舰队自己的数字过线。
3. **唯一还有算术空间的是②本身**:564 次引用/月是舰队唯一「大站级」的指标。
   按 €0,05/次 计就是 €28/月,超过 eco 整条 Amazon 线。**所以突破口不是把引用变成
   点击,而是进入按答案/按归因付钱的市场。** 那个市场 2026 年才出现且尚未开门到本量级
   (Cloudflare Pay Per Crawl **闭 beta**;Pay Per Use「2026 年晚些时候」无日期;
   ProRata/RSL 按归因分成但费率不公开)。**结论:GEO 是期权,不是渠道** ——
   成本压到零、可见度做到最大、仪表建起来、开门时第一批进去;**今天为它写新页、
   开新站、加新基建一律负收益**。

**卖家立场(以本轮 1 067 与 564 两个自有数字为依据,永久)**:
**只做「按答案付费」的卖家,永远不做「按抓取付费」的卖家。** 给引用爬虫定价 = 它们收 402
= 不索引 = 引用份额归零;而即使按 $0,10/次 这个没人同意过的激进价,1 067 次也只值
$106,70/28 天。保险丝「引用队列爬虫任何时候不设价不屏蔽」优先级高于一切变现提案。

**已建(零 owner 动作)**:`tools/geo_ledger.py`(供给/需求/转化/钱/pay-per-crawl 天花板,
`--selftest` 16 组断言、已验证能红)→ `data/geo-ledger.json`,**挂进 `fleet-heartbeat.yml`
不新增 schedule**;退出码 `2` = 五家引用爬虫任一沉默 >10 天 → run 变红 → owner 邮件
(这就是 2026-09-15 Cloudflare 默认拦截的自动熔断,取代原来那条备忘录)。
**同时修掉一个潜伏一个月的仪表盲区**:agi 的 `ua_audit` 只存 48 字符 UA 前缀,而 GPTBot 的
UA 前 48 字符与另外几个 bot 完全相同 —— 28 天 **4 114 次**抓取落在一个无法辨认的桶里,
`LIKE '%GPTBot%'` 查 90 天返回 0,**那个 0 是仪器读数不是事实**。bot 行已改为 120 字符
(人类/JS 仍 48,浏览器 UA 是指纹面)。首份真实台账 2026-09-10 08:00 UTC 落库
(本会话沙箱到 api.cloudflare.com 是 403,**不放手工拼的种子文件**)。

**判定线(预登记)**:①**2026-09-17** 台账连续 7 天落库且 `unidentified_hits` 较 4 114 显著下降,
否则修仪表不谈结论;②**2026-10-07**(仪表 +28d)任一站 AI 引荐 pv 占比 >5% **或** eco 的
AI 转化率 ≥11%(搜索基线一半),否则「GEO 是渠道」正式判负、此后只维持零成本可见度;
③每日:五家引用爬虫无一沉默 >10 天(heartbeat 自动);④**2026-12-05** 按答案付费任一渠道
>€0,否则 GEO 变现整体归档、不再第三次提起。

**owner 决策卡(按信息价值排序)**:①**Bing AI Performance 两张 Download all 截图(1 分钟)**
——引用数停在 08-16 的 564,不刷新则链路第②段无法结算,这是卡上信息价值最高的一项;
②Cloudflare `paypercrawl-signup` 填表**排队**(1 分钟,**不要在后台给任何爬虫设价或 Block**);
③ProRata / Gist Answers 出版商申请(5 分钟,按归因分成,正是舰队的形状);
④(承接 09-05)Comet Plus 邮件;⑤(承接 09-05)Metaculus 算力到手后先手动 dispatch 再设
`METACULUS_BOT_ENABLED=1`;⑥(承接 08-25)PartnerNet 付款/税务信息。

**本轮新增杀单(别再提)**:为 GEO 新建数据站/子站/子域 · 给爬虫定价 ·
「AI 流量转化更高所以押注它」(实测方向相反)· 「上 MCP/llms.txt 助手就会来用」
(eco 四周实测可归因助手调用 **0**,`mcp_call` 471 次里 252 次的参数与舰队自己的
`mcp_smoke.mjs` 逐字一致、108 次空参数目录探活 —— **MCP 调用数不得作为增长指标引用**)·
用引用数当营收先行指标(564 ↔ €0 已连续三个月同向,引用只能当可见度指标)。

## 股票板块「年化 100%」目标:先算清充要条件,再谈算法(2026-09-09,owner:「股票板块子站点…要做到以年化100%为目标」+「优化prompt再执行:建一个股票预测模型,合理调研股票算法,如果没有合适的,你创造一个,实现收益100%」)

**裁定全文 `docs/stock-model-100pct-2026-09.md`。目标有精确答案,而且不由算法决定。**
杠杆 L 下复利增长率 `g(L)=L·μ−L²σ²/2` 是**开口向下的抛物线**:Kelly 处见顶,**再加杠杆更低**,
2 倍 Kelly 处增长率为 0。所以任何杠杆能达到的最高年化 = `exp(S²/2)−1`,**只看夏普 S**:

| 夏普 | 0.4 | 0.6 | 0.8 | 1.0 | **1.177** | 1.3 | 2.0 |
|---|---|---|---|---|---|---|---|
| 天花板 | 8,3% | 19,7% | 37,7% | 64,9% | **100%** | 132,8% | 638,9% |

**年化 100% ⇔ 夏普 ≥ 1,177(满 Kelly)/ 1,360(半 Kelly)**,未计融资、手续费与税。
**现实参照:Medallion 1988–2018 费前 66%/年、费后 39%/年,1993 就对外封闭 [thin] —— 史上最强
量化基金自己都没持续做到净 100%。** 唯一越过 1,177 的公开数字是 Gu-Kelly-Xiu(RFS 2020)
多空十分位组合的 **1,35**,前提是 **900+ 特征、全美股截面、做空底部十分位、未扣成本**,
四条舰队一条都不满足;同一篇论文的月度**样本外 R² 只有 0,26–0,40%** —— 这就是股票可预测性的量级。

**已建(零 owner 动作,纸面,2026-09-09 预登记 / 2026-09-10 起跑)**:
- **`target_100` 算术层**:每根臂每天报出自己的夏普、任何杠杆下的天花板、100% 是否结构上不可达;
  **不足 60 个交易日输出 `insufficient_history`,绝不给噪声夏普**。已修一个真缺陷:第一版对
  **亏钱**的臂也报天花板(`S²` 丢符号,年化 −12% 显示「天花板 63,9%」),现在负超额直接归 0。
- **`tqqq_trend` / `tqqq_hold`**:真实 3 倍纳指基金,一根带 200 日趋势过滤(信号取 **QQQ 指数**,
  不取杠杆基金本身)、一根裸持作对照。用真实复权价,融资/管理费/每日重置衰减已在价格里,零假设。
  **代价必须同屏公布**:TQQQ 2021-11→2022-12 最大回撤 **−81,66%**,同期 QQQ 约 −32,7%,
  回本需 +376%、约 486 个交易日 [thin]。
- **`ml_ridge`(自己写的模型)**:10 只 AI 篮子的**截面岭回归**,5 个特征(12-1 动量、6-1 动量、
  1 月反转、21 日已实现波动、距 200 日均线偏离),**在 10 只之间做 z-score**,用所有前向收益已实现的
  历史月份拟合(λ=10 上线前固定),取预测为正的前三等权,一个都不为正则持 BIL。**纯标准库**
  (5×5 高斯消元 + 岭正则,无 numpy/sklearn)——跑不动的模型不是证据。价格历史 430→**1100 天**
  以喂训练样本(约 240 个股票-月);样本 <120 观测时**持现金不猜**。
  **前视泄漏有能红的测试**:把截止日之后的价格乘 10,训练集必须逐位不变;去掉守卫立刻失败(已实测)。

**红线**:三根新臂**永不进真钱执行器** —— `PAPER_ONLY` 常量 + 一条 selftest 断言读
`alpaca_mirror.py` 源码核对;既有 11 臂的规则与起始日**一个字不改**(新臂另起 09-10,零前视)。

**判定线(预登记)**:①**2026-09-24** 起跑 +10 交易日管道健康;②起跑 +60 交易日(约 **2026-12-04**)
`target_100` 从 `insufficient_history` 变 `measured`,没数据就说没数据、不补估计;
③**2027-03-08**(与既有读数日同日)`ml_ridge` 夏普须高于 `agi_basket` 与 `basket_mom5` 中较高者,
否则判「拟合模型在本舰队约束下不优于公开规则」,**不加参数不换算法**;
④**2027-03-08** 若无任一臂测得夏普 ≥1,177,如实写「未测到任何一根臂具备年化 100% 的结构条件」。

**本轮新增杀单(别再提)**:「换个更好的算法就能到 100%」(天花板只看夏普,夏普 1.0 封顶 64,9%;
再提须先给出能持续 ≥1,177 夏普的样本外证据)· 「加杠杆到 100%」(超 Kelly 增长下降,
2 倍 Kelly 为 0)· 「深度学习/LSTM/Transformer 选股」(公开最好月度样本外 R² 就是 0,3–0,4%,
且依赖 900+ 特征与做空)· 「调参提高收益」(发表后衰减 −58%;`ML_RIDGE_LAMBDA` / `ML_TOP_K`
上线前固定,**任何上线后的调整都作废该臂并重新预登记**)· 回测当证据(沿用 09-05)。
