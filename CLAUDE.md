...
## 股票算法深度调研(第二轮):最新神经网络/AI 全部落榜,只留一个对照实验(2026-09-09,owner:「先完善prompt再执行:再深度调研股票算法,最新的神经网络与ai算法,应用在股票上」)

**裁定全文 `docs/stock-ai-algorithms-2026-09.md`(三轮 prompt、按算法族的证据表、事实表全带
可信度与沙箱限制)。2024–2026 这一波在股票上论文极多,但「样本外 + 扣成本 + 对上简单基准」
三条同时成立的结果一个都没有。**

**最硬的一行是真钱对照**:AI 选股 ETF **AIEQ**(2017-10-17 成立,公开申报)至 2026-05-31,
$10,000 → **$23,321**,同期标普 500 全收益 **$33,857** —— 约 **10,3%/年 vs 15,2%/年**,
**真钱跑了 8 年半,输指数约 4,9 个百分点/年** [thin]。**任何「用最新 AI 算法炒股」的新提案,
必须先回答自己凭什么比 AIEQ 强。**

**按算法族的落榜理由(逐条,后续会话别再逐个重提)**:
- **Transformer 系**:同族内部结论互相矛盾——DLinear(单层线性)在标准基准上打平或胜过
  Transformer 预测器,FITS 用 1 万–5 万参数打平 PatchTST;参数量不是瓶颈,信噪比才是。
- **时序基础模型**(TimeGPT/TimesFM-2.5/Moirai-2.0/Chronos):2026 基准里 10 项拿 8 项第一,
  但**对随机游走的增益「小而稀疏」**,DM 检验仅 2 处拒绝;**作者自己的结论是「有用的先验,
  不是可靠的 alpha 引擎」**。
- **图神经网络**:10 只股票构不成有信息量的图;公开的 4%–15% 提升报的是分类准确率不是净收益。
- **深度强化学习**:超额收益不跨市场稳健(仅 Euro Stoxx 50 显著),且被交易成本吃掉。
- **LLM 交易代理**:**StockBench**(截止日之后的 2025-03→07 道指)多数代理跑不赢买入持有;
  **PortBench** 30 个组合有 **27 个跑不赢等权**;更根本的是 **LLM 权重里的前视偏差**
  (能逐字背出截止日前的标普价格)让干净样本外回测**原理上不可能**。
  → **舰队的 `llm_agent` 臂恰好是文献说缺的那种设计(前向、上线后决策、日期公开),
  保持不变,不加多代理/工具/记忆架构。**

**已建的唯一实验(零 owner 动作,纸面,2026-09-09 预登记 / 09-10 起跑)**:
- **`ml_gbm`** 梯度提升**深度 2** 决策树。**与 `ml_ridge` 唯一的差别是函数类**:同样 5 个特征、
  同样 10 只股票、同样截面 z-score、同样月频、同样「取正预测前三等权,全负则持 BIL」。
  超参上线前固定(50 轮、lr 0,05、叶最小 8)。**深度必须 ≥2**:深度 1 的树桩集成是可加的,
  表达不了交互,用它做对照等于什么都没测(有断言,注入深度 1 立刻红)。纯标准库,全台账 0,8 秒。
  **它回答的是一个没人在这个量级上回答过的问题**:GKX 把树/NN 的增益归因于**变量间非线性交互**,
  但那是全美股截面 900+ 变量测出来的——**能不能下沉到 10 只股票 5 个特征,本臂来答。**
- **`model_scores` 记分板**:把两个模型放进**论文自己的单位**——月度样本外 R² 与秩 IC,
  同块带 GKX 的 0,26%(线性)/0,33–0,40%(树·NN)作参照及「他们是全截面 900+ 变量」的口径。
  **前向 only**:早于 09-10 的月份永不打分,forward window 未完成的月份不打分(有断言);
  不足 6 个月输出 `insufficient_history`;**与交易臂共用同一条预测代码路径**——两份代码就意味着
  公布的分数不再描述公布的组合。**整份台账没有任何回测曲线。**

**判定线(预登记)**:①**2026-09-24** 记分板落库且自检绿;②约 **2027-03-31** 满 6 个月后
`status` 变 `measured`,没数据就说没数据;③**2027-03-08** `ml_gbm` 的 R² **与**秩 IC 都要高于
`ml_ridge`,否则判「本舰队规模上非线性不加分」,**此后不再引入任何新模型族**
(Transformer / GNN / DRL / 基础模型一律不建);④同日两模型秩 IC 与 R² 若均 ≤0,
判「五特征十股票的截面预测在本站无信息」,两臂降级为历史记录,**不再加特征**。

**沙箱口径**:本轮 arxiv.org / stockanalysis.com 等 egress 403,标 [thin] 的条目**只有检索摘要、
没读到一手页**;引用时照此说明,不要假装读过原文。

## 股票方向变成可售卖工具:卖尺子不卖预测(2026-09-11,owner:「继续搞清楚这个方向,变成一个工具,并可以售卖」)

**裁定全文 `docs/strategy-audit-product-2026-09.md`。这个方向里可售卖且合法的部分不是预测,是测量。**
预测那半边已被三份文档钉死(月度 OOS R² 0,26–0,40%;AIEQ 真钱 8 年半输指数约 4,9 pp/年;
年化 100% 需夏普 ≥1,177 而 Medallion 费后 39%),而且**在德(BaFin WpIG)、美(RIA/BD)、
中(投顾牌照)全是牌照问题**,`/ai-trading-ledger` 红线也明令不做信号服务。
**「这份业绩记录算不算数」则是算术、不受监管、且几乎没人在卖**——所有回测平台都在帮你**造**回测。

**监管边界就是一句话**:有没有**对某个金融工具**给出建议。本工具读用户**自己已有的一列数字**,
返回**关于这列数字的统计量**,不命名标的、不预测、不建议。这条写进代码注释、README、页面 FAQ、
`.md` 镜像四处,**后续会话不得放松**。

**已建(零 owner 动作)**:
- **`tools/strategy-audit/audit.py`** 单文件零依赖,~35 条自检。输出五组数:概率化夏普(PSR)、
  **通缩夏普(DSR,先减掉「试 N 次本来就会出现的高度」)**、最短可判业绩长度(MinTRL)、
  目标可行性(所需夏普 + `exp(S²/2)−1` 天花板)、成本折损。方法出处 Bailey & López de Prado 2014。
- **`/backtest-audit`** 免费网页,**纯浏览器运行、数据不出本机**(这既是隐私承诺也是产品定位)。
  已接线:sitemap、search-index、llms.txt、`/ai-tools`(第 10 个工具)、`.md` 镜像。
- **跨实现对等测试** `test_parity.mjs`:**从已发布页面里抽出那段 JS**(测的就是发出去的那个),
  与 Python 在 7 个确定性用例比对 **129 个字段**,容差 2e-6,**挂在 agiscorecard 部署路径上**。
  它当场抓到一个真缺陷:净值粘进「收益率」框时 Python 端 `math.exp` 溢出崩溃、JS 端打 Infinity;
  现在两侧在夏普 >10 时判定为单位错误并停止出数,各有断言守着。
- **演示用例就是标普自己**:14 个月 SPY 年化夏普 **1,63**(4 倍杠杆即可复利 100%/年),
  但**若声明是试了 200 次才找到它,通缩夏普只有 0,16**。指数当然没过拟合——这正是重点。

**售卖路径:不新建任何基建。** 免费工具是**需求入口**,付费的是 `/audits` 上**已存在**的定价审计
(USDT/发票,E6 预登记 $500–2000),新增一类「业绩主张审计」。不建订阅、不建结账、不开新域
(owner 不做销售 + 铁律「产品不存在之前不挂价」)。

**⚠️ 本轮纠正一个台账口径**:`/audits` 的「0 询单」**不是需求读数,是仪器缺失**。实查两条:
① **`audit_click` 从来不在采集器白名单里**(与 2026-08 `affiliate_click` 同款陷阱,页面发了也被丢);
② **`/audits` 整页没有任何联系动作可点**(全页 3 个 `subscribe_click` + 一个 `/advertise` 脚注链接)。
**那个 0 证明的是「没有东西可点」。** 2026-08-29 判死的「audits 表单重构」**杀单继续有效,本轮不碰那页**;
只做两件不越界的事:`audit_click` 入白名单 + 新页带一个真 CTA,**让这条线第一次有分母**。

**判定线(预登记)**:①**2026-09-18** 页面 200、对等测试每次部署绿、`calc_use{backtest_audit}` ≥1 真人;
②**2026-10-09**(+28d)`calc_use{backtest_audit}` **≥20 次真人**,否则判「需求入口不成立」,
页面降级为 `/ai-trading-ledger` 的一节、**不做第二个工具**;③**2026-11-11** 出现任一
「deflated sharpe / backtest overfitting」类搜索或 AI 引荐,否则不进引用放大队列;
④**2026-12-11**(+90d)≥1 次可归因的 `audit_click{backtest_audit}` 询单,否则判「卖审计」路负,
工具留作免费引用资产,**不再第三次提起把它变现**。

**永久红线(与 `/ai-trading-ledger` 同级)**:不出交易信号、不给「买 X」结论、不接券商、
**不做 zh 镜像**(中国监管)、**不收用户数据上传**。任何修改越过「只对用户自己的数字做统计」
这条线,该页立即下架。

**本轮新增杀单(别再提)**:卖预测/信号/选股工具(三地牌照)· 自建订阅或结账(owner 不做销售,
`/audits` 收款面已在)· 做第 N 个回测引擎(vectorbt PRO $20/月、QuantConnect $10–96/月,
功能面宽得多,正面竞争无胜算)· 需求验证前挂价 · 把工具做成需要上传数据的服务。
