...
2026-09-19 10:33 UTC，最终源码 `f2187f7b76ebaae9df5e43bfef8f1349ac7f032a` 部署成功，Worker 版本为 `a6c3de86-d2c8-4134-b25b-258df7ba5455`。[最终 Web3 部署运行](https://github.com/f-tiger/agi-site/actions/runs/35437697176) 再次通过全部测试和 11 主机/251 文件比对，本轮合计 354 次检查请求。

激励页新增奖励后的经济账表。浏览器复核：收入 80、成本 75、奖励 30，所以不含补贴余额 -25；补贴 100 后为 75。对账页将第二张发票改为 60 后重算，匹配数从 1 变成 2；输入空发票数组会显示错误并禁用下载。

[主站 Agent 入口](https://agiscorecard.com/for-agents) 的新 hub 链接与协议 JSON 链接已在浏览器中确认。[主站部署运行](https://github.com/f-tiger/agi-site/actions/runs/35437697162) 从当前页面重新生成 feed 和机器可读镜像，通过站点验证后部署；没有运行已过期的 gen_for_agents.py。
