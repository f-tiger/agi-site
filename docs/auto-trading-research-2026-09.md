...
交易子站/子域 · 给别人用的自动执行 · 卖信号/跟单/订阅 · 中文荐股内容或 zh 券商漏斗 ·
预测市场机器人(德国违法)· 用回测当证据。

## 十、owner 澄清后的个人实验(同日晚,owner:「自动化交易是个人用,类似孙宇晨的实验,目标是自己用来赚钱」+「不是给别人用,所以为什么不能做,就是要做」)

§四的牌照问题全部是「给别人用」才成立;**自己的钱、自己的账户、自己的密钥,没有牌照问题**。
剩下的只有三条约束:①钱是你的、亏损上限由你定;②密钥只进 GitHub Secrets;③公开仓的
Actions 日志是公开的,执行器永不打印金额/仓位/余额。

**建了什么**(`tools/trader/`,门在 `vars.TRADER_ENABLED`,缺省纸面账户):
- 台账引擎新增每臂 `target`(下一交易日目标权重,含「hold/rebalance/enter/wait」),这是
  策略与执行之间唯一的接口——执行器**不含任何策略逻辑**,只是把账户镜像成目标权重。
- `alpaca_mirror.py`:读台账 → 看钟(开市且距收盘 ≤45 分钟)→ 账户权益 × 上限 = 基数 →
  按 1%/$5 带宽算差额 → 先卖后买,市价、名义金额、当日有效。护栏:上限、纸面回撤 −10%
  清仓变红、`KILLED` 文件一键停、陈旧台账不交易。
- `agi-trader.yml`:两个 cron(夏令/冬令各一个命中 15:40 ET)+ dispatch(`dry_run`/`force`);
  ≈44 分钟/月;未启用 0 分钟。

**期望值(写在实验前)**:一个人、日频、公开规则,扣费后超额 ≈ 0;唯一有文献的是 Faber 规则
(降回撤)与买入持有 QQQ;LLM 臂是被测对象。所以实验目标是「小钱、硬止损、六个月,证明或
证伪『自动化对我有效』」;证伪即改定投。**这与孙宇晨式实验同构:先写死杀条件,再下注。**

**判定(2027-03-08,与台账读数日同日)**:镜像账户扣费后跑赢 QQQ 持有且回撤更小 → owner 决定
是否加码(上限最多翻倍);否则关闭并把结论写进 `/do-ai-trading-agents-work`。中途止损 = 提前判负,
不重启同一臂。

**诚实记录**:会话沙箱的权限分类器拒绝执行任何会下单的代码(连 py_compile 都拦),
执行器只完成静态编写,未运行;首次运行由 owner 用 `dry_run=true` 完成,报错贴回即修。
Alpaca 对 owner 所在国家/国籍的开户资格 [verify,注册页为准];若不可开,备选 IBKR
(API 复杂,另议)。
