...
| `fleet-trends.yml` | `45 3 * * *` | 全舰队趋势快照 **2026-09-12 雷达加 Reddit 求做板块源**(r/SomebodyMakeThis + r/AppIdeas 公开 JSON,只读,增量 <0,2 分/月)。**2026-09-13 加机会撮合层**:`tools/fleet/opportunity_match.py`(零 AI,Reddit 请求 × rising × PH/HN 供给 → `data/autopilot/opportunities.json`)+ `sites/buysomething/tools/gen_demand_board.py`(出页门 ≥3,不转载 Reddit 内容);秒级;页有变化才触发 buysomething 部署(≈1 分/次)。裁定见 `docs/reddit-opportunity-board-2026-09-13.md` |
