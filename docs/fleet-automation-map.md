...
2026-09-18 实时诊断补充：`growth-diagnose.yml` 仅由本 PR 分支上的诊断脚本/工作流变化或手动触发，权限 contents:read，固定聚合查询，8 分钟上限、artifact 7 天，无 schedule、无部署。D1 读取失败明示 warning，公共接口回退不冒充完整 D1。Owner 确认 private 是主动选择，额度不足再切 public；下文“公开仓免费”为历史运行状态，不能把当前可见性当增长故障。

2026-09-18 增长重置补充：`bpj-growth-check.yml` 只在相关 PR 上运行无密钥浏览器回归，预计 1–3 分钟/次、上限 6 分钟，无 schedule。部署现有工作流加 `test-hit-events.mjs`，heartbeat 现有 AI 引荐步骤加解析自检，均秒级；`reach-export` 沿用每日任务导出 `growth`，未新增定时任务。流量口径与投入分配以 [增长方案](growth-reset-2026-09-18.md) 为准，不再把混合采集总量称为真人数。
...
| `deploy-firstjob.yml` | firstjob.agiscorecard.com | **`35 8 * * *`（2026-09-15 建站，同 `/goal` 方向候选 #2 毕业生就业）** |
| `deploy-codeword.yml` | codeword.agiscorecard.com | **`50 8 * * *`（2026-09-15 建站，owner「扩展几个适配欧洲、美国的站点」；英文反诈判定）** |
| `deploy-powerbill.yml` | powerbill.agiscorecard.com | **`5 9 * * *`（2026-09-15 建站，同一指令；美欧电费判定）** |
