...
## 两层动作模型(guardrails,2026-08-21 二次修订)

- **自动层**:选材、起草、暂存、状态记录。
- **人工层**:一切发布(HN/Reddit/X)。HN/Reddit 是「不可为」(代发烧号毁渠道);
  X 的代发管道(tools/x-poster/)已建成并验证到 X API 应答,但 X 自 2026-02-06
  起对新开发者取消免费档、带链接帖 $0.20/条(≈$7.4/月),owner 当日决定不为零
  粉丝存在层渠道付固定成本 → **管道休眠**(代码/队列/密钥保留,cron 已摘,
  充值或免费档恢复时一行重启)。X 内容照常起草进周暂存文件,由 owner 在 App
  里免费手发。
