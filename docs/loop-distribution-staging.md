...
## 两层动作模型(guardrails,2026-08-21 修订)

- **自动层**:选材、起草、暂存、状态记录,以及 **X 渠道的发布本身**——owner 当日
  授权破「机器不代发」,仅限 X 官方 API + owner 本人账号:`tools/x-poster/`
  worker 每日 13:30 UTC 从 queue.js 发一项(280 门禁、KV 防重发、3 次失败熔断、
  无密钥休眠)。周循环把本周最强 1-2 条直接追加进队列。
- **人工层**:HN 与 Reddit 的发布。这两个平台的代发会烧号并永久毁掉渠道,
  不属于「可自动化」而属于「不可为」——**此线不随本次修订移动。**
