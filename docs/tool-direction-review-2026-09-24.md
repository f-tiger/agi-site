...
3. **安装这一步的读数 —— 本轮已建,并修正了我自己上一段的说法**:eco 的 `mcp_install_click` 在 D1 里 0 行,
   但原因不是「没人点」:**它的页面只把这个事件发给 GA4,压根没发给 D1**(`sites/getecoback/site/mcp.html`
   那行 `gtag("event","mcp_install_click",…)`)。也就是说这一步在整个舰队**从来没有被测过**,不是测出来是 0。
   本轮在 SR 的 `/mcp` 上补齐:两条可复制的安装指令(Claude Code 一行命令 / 通吃的 JSON 配置)+ 复制时
   落一行 `mcp_install_click`(只记客户端名,零 PII)+ `/e` 白名单放行 + 部署自检断言页面上这两样东西还在。
   **这不是新工具,是把「被发现 → 被装上」之间那一步变成可读的数。** 判定线 `sr-mcp-install-1022`。
...
- **`sr-mcp-calls-1014` / `sr-vertical-1014` / `fleet-tool-use-1014`**:各写入 09-24 中期读数,
  **不提前结算**(10-14 到期按原文办)。
- **新增 `sr-mcp-install-1022`**(10-22):SR 的 `mcp_install_click` 28 天 ≥1。
  win → 这一步可测,把同样的埋点补给 eco/bpj;lose → 连复制安装指令的人都没有,机器面对人的入口不是页面,
  停止在页面上投入,只留注册表与 llms.txt 两条被动发现。
