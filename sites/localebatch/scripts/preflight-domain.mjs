...
const zoneSecret=process.env.CLOUDFLARE_API_TOKEN_ZONE||secret;
if(!account||!secret)throw Error('Existing Cloudflare deployment credentials are unavailable.');
async function api(path,token=secret){const r=await fetch('https://api.cloudflare.com/client/v4/'+path,{headers:{Authorization:'Bearer '+token},signal:AbortSignal.timeout(15000)});const d=await r.json();if(!r.ok||!d.success)throw Error('Cloudflare '+(path.startsWith('zones')?'zone':'Worker')+' read-only preflight failed: HTTP '+r.status);return d.result;}
...
if(match){console.log('Hostname is already assigned to this Worker.');}
 ...
 const zones=await api('zones?name=agiscorecard.com',zoneSecret);
 if(zones.length!==1)throw Error('Expected owner zone was not resolved.');
 const records=await api(`zones/${zones[0].id}/dns_records?name=${host}`,zoneSecret);
