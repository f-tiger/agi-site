...
const error=(message,status,code='request_rejected',detail)=>Response.json({error:message,code,...(detail?{detail}: {})},{status,headers:{'Cache-Control':'no-store'}});
export async function secConcept(request,env,fetcher=(...args)=>globalThis.fetch(...args),cache=globalThis.caches?.default){
 ...
 try{
  const r=await fetcher(source,{headers:{'User-Agent':'AGI Scorecard FilingLens research contact https://agiscorecard.com','Accept':'application/json'},signal:controller.signal,redirect:'manual'});
  ...
  return response;
 }catch(e){return error('Official data could not be validated or the request timed out. No substitute data was used.',502,stage+'_failed',stage==='source_fetch'?{kind:String(e?.name||'Error'),message:String(e?.message||'Source request failed').replace(/https?:\/\/\S+/g,'[official source URL]').slice(0,160)}:undefined);}finally{clearTimeout(timeout);}
}
