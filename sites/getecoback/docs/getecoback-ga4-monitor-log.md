...
## 2026-08-21 数据卫生规则(永久,读数前先看这条)
`ev` 表里 `page='/__ci_healthcheck'`(meta 含 `"ci":1`)的行是 deploy 工作流的
漏斗断言自打点,**每次部署都会产生 lead_intent + cold_now 各一条**。任何判定线
统计(lead_intent ≥10/28d 等)与日常读数必须加 `AND page != '/__ci_healthcheck'`。
今日已实测踩坑一次:部署频繁的日子里 CI 行会累积成看似真实的信号(今天 2 组、
