...
- **部署后内容断言会撞边缘缓存**(run 231 红):部署后 15 s 内,边缘可能还用旧版回答(`cf-cache-status: HIT`)。新页面的内容断言因此读到旧版,而几秒后线上已经是新内容。`check()` 以前只在非 200 时重试,现在内容缺失时每 10 s 重试一次、最多 3 次,约 30 s 后仍缺才判红;中途从旧版变成新版会打一条 `::notice::`。**新增内容断言照旧写,不要为了它去改缓存头。**
