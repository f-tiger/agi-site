...
for lang,t in COPY.items():
 ...
 nextlinks=''.join(f'<li><a data-next-step="{esc(v["id"])}" href="{esc(v["path"])}">{esc(v["label"])}</a><p>{esc(v["hint"])}</p></li>' for v in t['nextLinks'])
 for v in t['nextLinks']:assert (SITE/v['path'].lstrip('/')).exists(),v['path']
...
<section id="next-steps" class="panel next-steps" hidden aria-labelledby="next-heading"><h2 id="next-heading">{esc(t["nextTitle"])}</h2><p>{esc(t["nextIntro"])}</p><ul>{nextlinks}</ul></section>
