...
<div class="actions"><button type="button" id="share">{esc(t["share"])}</button><button type="button" id="copy-forum">Forum text</button><button type="button" id="download-card">PNG</button><button type="button" id="print">{esc(t["print"])}</button></div><div class="community-share" aria-label="Community sharing"><span>{esc(t["share"])}:</span><a data-share="whatsapp" target="_blank" rel="noopener">WhatsApp</a><a data-share="reddit" target="_blank" rel="noopener">Reddit</a><a data-share="x" target="_blank" rel="noopener">X</a><a data-share="email">Email</a></div><p id="share-status" role="status"></p><a id="share-link" hidden></a></section>
</div>
<article>
{body}
