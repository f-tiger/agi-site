#!/usr/bin/env python3
"""hreflang 规范化 + 门禁（幂等）。方法论：seo-hreflang 技能，2026-08-17 审计落地；
2026-08-28 意大利语试点将「配对(2页)」模型升级为「语言组(≤3页)」模型。

审计查出的三类真缺陷（都在 DE↔EN 配对页上，手写标签各写各的漂移出来的）：
  ① 同一配对两侧 x-default 指向不同 URL（一组 alternates 只允许一个 x-default）；
  ② x-default 只挂了一侧；③ 部分配对整组没有 x-default —— 站内约定不一致。

本脚本把「多语言组的 hreflang 块」变成生成物，不再靠手写：
  - 组关系从各页已声明的 alternate 标签取并集推导（任一侧声明即入组，双向自愈）；
  - 页面语言由路径判定：en/ → en，it/ → it，其余 → de；一组内每种语言至多一页；
  - 组内所有成员统一重写为同一块：按 de→en→it 顺序每语言一行 +
    x-default → 组内优先级最高的语言（de > en > it；DE 是主市场）；
  - 单语页（仅自引用）不动，只校验；
  - 校验失败退出码非零 = 门禁：语言码白名单 / href 绝对 https / 目标文件存在 /
    canonical 存在且自引用与其逐字一致 / 同组同语言唯一。

幂等：重复运行 byte-stable（原 de/en 配对的输出与旧脚本逐字相同）。
接入 deploy.yml（xlinks 之后、sitemap 之前）。
"""
import re
import sys
from pathlib import Path

ROOT = Path(__file__).resolve().parent.parent
SITE = ROOT / "site"
BASE = "https://getecoback.com"

TAG_RE = re.compile(r'[ \t]*<link rel="alternate" hreflang="([^"]+)" href="([^"]+)">\n?')
CANON_RE = re.compile(r'<link rel="canonical" href="([^"]+)">')
VALID_LANGS = {"de", "en", "it"}  # ISO 639-1；本站不用地区码（受众是语言不是国家）
LANG_ORDER = ["de", "en", "it"]  # 块内行序 = x-default 优先级


def url_to_file(url: str):
    if not url.startswith(BASE):
        return None
    path = url[len(BASE):]
    if path in ("", "/"):
        path = "/index.html"
    if path.endswith("/"):
        path += "index.html"
    return SITE / path.lstrip("/")


def lang_of(f: Path) -> str:
    rel = str(f.relative_to(SITE))
    if rel.startswith("en/"):
        return "en"
    if rel.startswith("it/"):
        return "it"
    return "de"


def main() -> int:
    pages = {}  # file -> {"html": str, "canonical": str|None, "tags": [(lang, href)]}
    errors = []
    for f in sorted(SITE.rglob("*.html")):
        html = f.read_text(encoding="utf-8")
        tags = [(m.group(1), m.group(2)) for m in TAG_RE.finditer(html)]
        m = CANON_RE.search(html)
        pages[f] = {"html": html, "canonical": m.group(1) if m else None, "tags": tags}

    # ── 推导语言组：任一页声明了指向另一个真实文件的合法 alternate 即连边（并查集）──
    parent = {}

    def find(x):
        parent.setdefault(x, x)
        while parent[x] != x:
            parent[x] = parent[parent[x]]
            x = parent[x]
        return x

    def union(a, b):
        ra, rb = find(a), find(b)
        if ra != rb:
            parent[rb] = ra

    linked = set()
    for f, d in pages.items():
        for lang, href in d["tags"]:
            if lang == "x-default":
                continue
            if lang not in VALID_LANGS:
                errors.append(f"{f.relative_to(ROOT)}: 非法语言码 hreflang=\"{lang}\"")
                continue
            if not href.startswith("https://"):
                errors.append(f"{f.relative_to(ROOT)}: hreflang href 必须是绝对 https URL：{href}")
                continue
            target = url_to_file(href)
            if target is None:
                errors.append(f"{f.relative_to(ROOT)}: hreflang 指向站外 URL：{href}")
                continue
            if not target.exists():
                errors.append(f"{f.relative_to(ROOT)}: hreflang 死链 {lang} -> {href}")
                continue
            if target == f:
                continue  # 自引用
            union(f, target)
            linked.add(f)
            linked.add(target)

    if errors:
        for e in errors:
            print("HREFLANG-ERR", e)
        return 1

    groups = {}  # root -> [files]
    for f in linked:
        groups.setdefault(find(f), []).append(f)

    # ── 组校验：同组内每种语言至多一页 ──
    for members in groups.values():
        by_lang = {}
        for f in sorted(members):
            l = lang_of(f)
            if l in by_lang:
                errors.append(
                    f"{f.relative_to(ROOT)}: 同组内语言 {l} 重复（已有 {by_lang[l].name}）")
            else:
                by_lang[l] = f

    if errors:
        for e in errors:
            print("HREFLANG-ERR", e)
        return 1

    # ── 规范化组内页：全体成员写入同一块（按语言序 + x-default→最高优先语言）──
    changed = 0
    n_grouped = 0
    for members in sorted(groups.values(), key=lambda ms: str(min(ms))):
        by_lang = {lang_of(f): f for f in members}
        missing_canon = [f for f in members if pages[f]["canonical"] is None]
        if missing_canon:
            for f in missing_canon:
                errors.append(f"{f.relative_to(ROOT)}: 组内页缺 canonical，无法生成 hreflang")
            continue
        langs = [l for l in LANG_ORDER if l in by_lang]
        xd_url = pages[by_lang[langs[0]]]["canonical"]
        block = "".join(
            f'<link rel="alternate" hreflang="{l}" href="{pages[by_lang[l]]["canonical"]}">\n'
            for l in langs
        ) + f'<link rel="alternate" hreflang="x-default" href="{xd_url}">\n'
        for f in sorted(members):
            d = pages[f]
            html = d["html"]
            stripped = TAG_RE.sub("", html)
            canon_m = CANON_RE.search(stripped)
            if canon_m is None:
                errors.append(f"{f.relative_to(ROOT)}: 找不到 canonical 行，无法定位插入点")
                continue
            insert_at = stripped.index("\n", canon_m.end()) + 1
            new_html = stripped[:insert_at] + block + stripped[insert_at:]
            if new_html != html:
                f.write_text(new_html, encoding="utf-8")
                changed += 1
            pages[f]["html"] = new_html
            pages[f]["tags"] = [(m.group(1), m.group(2)) for m in TAG_RE.finditer(new_html)]
            n_grouped += 1

    if errors:
        for e in errors:
            print("HREFLANG-ERR", e)
        return 1

    # ── 终检（含单语页）：自引用逐字等于 canonical；x-default ≤1 ──
    n_solo = 0
    for f, d in pages.items():
        tags = d["tags"]
        if not tags:
            continue
        canon = d["canonical"]
        if canon is None:
            errors.append(f"{f.relative_to(ROOT)}: 有 hreflang 但无 canonical")
            continue
        hrefs = [h for _, h in tags]
        if canon not in hrefs:
            errors.append(f"{f.relative_to(ROOT)}: 缺自引用（无 alternate 逐字等于 canonical {canon}）")
        xds = [h for l, h in tags if l == "x-default"]
        if len(xds) > 1:
            errors.append(f"{f.relative_to(ROOT)}: x-default 多于 1 个")
        if f not in linked:
            n_solo += 1
            for l, h in tags:
                if h != canon:
                    errors.append(f"{f.relative_to(ROOT)}: 单语页出现指向他页的 alternate（{l} -> {h}）却未成组")

    if errors:
        for e in errors:
            print("HREFLANG-ERR", e)
        return 1

    sizes = [len(ms) for ms in groups.values()]
    print(f"hreflang OK：语言组 {len(groups)}（成员页 {n_grouped}，其中三语组 "
          f"{sum(1 for s in sizes if s == 3)}，本次重写 {changed}）· 单语自引用页 {n_solo} · 全部校验通过")
    return 0


if __name__ == "__main__":
    sys.exit(main())
