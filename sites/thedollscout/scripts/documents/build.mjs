...
const origin = 'https://thedollscout.com', edition = '2026-09-25.3', updated = '2026-09-25';
...
const breadcrumb = (lang, slug, title) => slug ? `<nav class="breadcrumb" aria-label="Breadcrumb"><a href="${route(lang)}">${esc(copy[lang].home)}</a><span aria-hidden="true"> / </span>${slug.startsWith('learn/') ? `<a href="${route(lang)}#guides">${esc(copy[lang].guides)}</a><span aria-hidden="true"> / </span>` : ''}<span aria-current="page">${esc(title)}</span></nav>` : '';
function shell(lang, slug, title, description, body, mode = '') {
  ...
  const guide = slug.startsWith('learn/'), socialImage = origin + '/img/document-scout-og.png';
  ...
  if (slug) {
    const items = [{ name:c.home, item:origin + route(lang) }];
...
for (const [lang, c] of Object.entries(copy)) {
  ...
  putPage(lang, '', c.homeTitle, c.intro, `<section class="hero"><div class="hero-copy"><p class="eyebrow">${esc(c.eyebrow)}</p><h1>${c.heroLines.map(line => `<span>${esc(line)}</span>`).join(' ')}</h1><p class="intro">${esc(c.intro)}</p><ul class="benefits">${c.benefits.map(v => `<li>${esc(v)}</li>`).join('')}</ul><p class="trust-line">${esc(c.resultScope)}</p></div>${workspace(c, 'audit', true)}</section>${results}${toolList}<section class="section"><h2>${esc(c.workflowTitle)}</h2><div class="workflow">${c.workflow.map(([h, p]) => `<article><h3>${esc(h)}</h3><p>${esc(p)}</p></article>`).join('')}</div></section><section class="free-band"><h2>${esc(c.freeTitle)}</h2><p>${esc(c.freeBody)}</p></section>${faq(c)}${guideList}`, 'audit');
