...
- **视觉锚点(2026-08-30 二改,owner:「现在的风格是不是太重了,不适合labubu的
  年轻人」)= Lo-Fi 纸感拼贴**:纸黄底、贴纸卡片(交替 ±0.8° 旋转 + 胶带角)、
  系统字体混排(Times 标题/Helvetica 正文/Courier 数字)、错色印刷效果只用在
  真实数字上("1:72" 贴纸、计算器结果)。工具 = 首页 C 位胶带剪贴板面板,
  js/odds-calc.js 四页共享(禁止再出现页内副本)。改视觉先过
  .claude/skills/frontend-design 的锚点纪律,别混搭。
