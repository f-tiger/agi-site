...
## AGIX v0.6 = 本站与后续网络的协调算法(owner 命名;site/agix.md 是唯一规则源,改规则必须升版本)
...
## v0.6 判定:传播机制不存在(2026-08-30,23-agent 对抗工作流,五个提案全部 2/2 被驳回)
owner 质问:「比特币可交易所以有人传播,agix 如何传播?」——**质问成立,且比我上一版承认的更严重**。
约 60 个 1994–2026 采纳案例的调研结论:**记录中不存在无代币版本的比特币传播引擎**。唯一命中率
100% 的机制是「一个有受众的消费者承诺读这个格式,并在发布当天给发布者可见回报」(搜索引擎→
sitemap/schema.org;拒收未申报库存的广告买家→ads.txt;有听众的客户端→RSS;首日即发布的客户端→
MCP);对照组 llms.txt 免费开放设计良好且已死,因为没有消费者承诺。更快的两股力量(gatekeeper 让
不发布变贵、platform 让发布成为默认)本站都不具备,也造不出来。
**因此 v0.6 删掉三条我自己写的、撑不住的说法**:①「registry 位置随协议传播而增值」——这正是
本文件 116 行「永不」清单里的「按传播升值的任何装置」,**我违反了自己写的宪法,这条记录必须保留**;
②「不可复制的资历」作为早采纳理由(只保留字面为真的窄形式:你无法追溯地发布一条带日期的判定);
③「审计需求」作为传播机制(审计需求是读者的结果,不是采纳的原因;本站 audit_click 终身为 0)。
「训练截止日=减半日」降级为**明确标注的未验证类比**。删除本身已按协议五字段挂上账本
(entry `our-own-protocol-economics`),这是规则对所有人的要求。
**一手实测(不许再被"人数"错觉污染)**:/claimledger.json 终身被外部客户端抓取 **0** 次;
/protocol、/agix、/grader.html、SKILL.md 终身 **0** 次加载;/ledger.json 的 10 次机器抓取
= 本仓部署自检 curl(10 次部署 ↔ 10 次抓取,精确吻合)——**自家 CI 不是采纳**,这是舰队第三次
抓到自测冒充需求。自检已加 `?ci=1`,worker 见 ci 直接不记账。
**保留**:诚实对齐激励(不依赖任何人采纳)、拒绝可转让性(代价照写:没有投机燃料就是更慢甚至没有)、
0% 永久协议费。**本站实际在赌的只有两件事**:①工具在零采纳时也对单个使用者有用 ②带日期的一手
判定被答案引擎引用。命名的依赖风险:**今天协议的所有消费者都是我们自己写的**(registry/grader/
SKILL/MCP)——RSS 不是被更好的格式打败的,是 Google Reader 2013-07-01 关闭后再没恢复。
判定:**2026-11-30 若仍无任何 /claimledger.json 被我们没写的客户端抓取,结论是协议没有消费者**,
如实记录,不再解释。
...
④(2026-08-30 增)`/skill/claim-ledger/SKILL.md` = **可安装 agent 技能**(开放 Agent Skills 标准——比 MCP 更轻的分发单元;
**「被 25+ 产品采纳」这个数字来自二手博客、未经核实,已从 protocol 公开面删除,别再写回去**):任何兼容 agent 装上
...
**2026-08-30 新增铁律**:①**不得再提任何"随采纳升值"的装置**,无论是否称为代币——v0.4 就是
这样越线的;②**不得把 registry 上架说成采纳激励**(无受众时价值≈0,说它有价值就是本站自己在
grade 的那类不可证伪主张);③**任何被点名方的判定页面旁不得出现付费入口**——首页曾把
「Claim Audit service →」直接放在 B.AI/PandaAI 判定下方,已删除并公布利益冲突规则
(「不向任何在册未结条目中被点名的一方出售审计/广告/任何服务」)+ 免费更正路径(公开 issue,
交带日期一手源,重算判定并保留旧判定可见);④**上架数不得作为成绩汇报**——llms.txt 正是在这个
数字上很好看而实际已死;只报「被我们没写的客户端抓取的次数」。
...
- **09-30 绊线(新增,最早的真相时刻)**:`/protocol` + `/agix` + `/grader.html`
  三页的 **JS 口径** page_view 合计 **> 0**(今天终身为 0)。仍为 0 = 瓶颈在分发不在
  经济学,**此后不许再写任何白皮书/协议提案**,只做被引用的判定页。
- **生存线 2026-10-28**:①JS 口径 page_view ≥50/28d(现已可测,beacon 已上线)
  或 ②出现 ≥1 个非 goldrush 的外部 referrer ——任一成立继续投入;全空 → 降为季度维护。
  (AI 引用仍可作为加分证据,但不再作为必需条件,因为沙箱取不到。)
- **协议采用线 2026-11-30**:①`/claimledger.json` 被**我们没写的客户端**抓取 ≥1 次
  (排除 goldrush referrer 与 `?ci=1`,定义见 /fetchlog.json)或 ②registry 的
  `ledger_render` 事件 label 为非 goldrush URL ≥1 次 ——任一成立传播引擎保留;
  全空 = **协议没有消费者**,如实记录,只剩引用机器。**上架数永不作为达标依据。**
