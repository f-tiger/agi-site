...
export async function writeManifest(){
 ...
 const manifest={};
 for(const s of [{id:'hub',host:hubHost},...sites]){const prefix=s.id==='hub'?'':s.id+'/';const files=(await readdir('dist/'+prefix,{recursive:true,withFileTypes:true})).filter(x=>x.isFile()).map(x=>posix.join(x.parentPath||x.path,x.name).replace(/^dist\//,''));const rootFiles=s.id==='hub'?files.filter(x=>!x.includes('/')||x.startsWith('.well-known/')||x.startsWith('zh/')):files;manifest[s.host]=Object.fromEntries(rootFiles.filter(x=>!shared.includes(x)).map(x=>{const route=x.slice(prefix.length);return[route==='index.html'?'/':route==='zh/index.html'?'/zh/':'/'+route,x];}).concat(shared.map(x=>['/'+x,x])));}
