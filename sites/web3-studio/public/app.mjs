...
const pretty=x=>JSON.stringify(x,null,2);let current=structuredClone(site.sample),report=null,ownCompleted=false,editingJson=false,activeCase=scenarios(id)[0].key,baseline=null;
...
function download(filename,text,type='application/json'){const blob=new Blob([text],{type}),url=URL.createObjectURL(blob),a=el('a');a.href=url;a.download=filename;document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),60000);$('status').textContent='Download requested. If your browser blocks it, use the report text below.';}
function chooseCase(key){const c=scenario(id,key);activeCase=c.key;$('scenario-picker').value=c.key;$('input-source').value='example';load(c.input,'Fictional scenario loaded.');for(const b of document.querySelectorAll('[data-scenario]'))b.setAttribute('aria-pressed',String(b.dataset.scenario===c.key));$('share-output').value=exampleLink(id,activeCase);$('editor-details').open=false;compute();}
$('load-example').addEventListener('click',()=>chooseCase(activeCase));
$('scenario-picker').addEventListener('change',()=>chooseCase($('scenario-picker').value));
for(const b of document.querySelectorAll('[data-scenario]'))b.addEventListener('click',()=>chooseCase(b.dataset.scenario));
document.querySelector('.secondary-cta').addEventListener('click',()=>{$('editor-details').open=true;$('clear').click();});
$('input-source').addEventListener('change',()=>invalidate('Input source changed. Run again.'));
$('clear').addEventListener('click',()=>{$('input-source').value='own';$('editor-details').open=true;const blank=Object.fromEntries(Object.entries(site.sample).map(([k,v])=>[k,Array.isArray(v)?[]:typeof v==='number'?0:typeof v==='boolean'?false:enums[k]?enums[k][0]:'']));load(blank,'Add your records and settings.');});
...
function compute(){try{const input=editingJson?$('json-input').value:pretty(current),out=run(id,input);report={tool:id,version:VERSION,createdAt:new Date().toISOString(),inputSource:$('input-source').value,result:out};renderResult(out);$('result-context').textContent=report.inputSource==='example'?'Fictional scenario · '+scenario(id,activeCase).title:'Your records · processed in this tab';renderComparison();$('report-output').value=pretty(report);$('export-report').disabled=false;$('copy-report').disabled=false;$('status').textContent='Review complete. The report is ready.';if($('input-source').value==='own')ownCompleted=true;$('result-title').focus();}catch(e){invalidate(e.message);$('status').focus();}}
...
function renderComparison(){const root=$('comparison-output');root.replaceChildren();$('clear-comparison').hidden=!baseline;if(!baseline)return;root.append(el('h4','Comparison kept in this tab'));if(!report){root.append(el('p','Run another scenario or edit the inputs to compare.'));return;}const wrap=el('div',undefined,'table-scroll'),table=el('table');table.append(el('caption','Kept result versus current result — same tool, supplied assumptions'));const head=el('thead'),row=el('tr');for(const name of ['Measure','Kept result','Current result']){const cell=el('th',name);cell.scope='col';row.append(cell);}head.append(row);table.append(head);const body=el('tbody');for(const m of report.result.metrics){const row=el('tr');row.append(el('td',m.label),el('td',baseline.result.metrics.find(b=>b.label===m.label)?.value??'Not available'),el('td',m.value));body.append(row);}table.append(body);wrap.append(table);root.append(el('p','Kept: '+baseline.context+' · '+baseline.createdAt,'quiet'),wrap);}
$('keep-comparison').addEventListener('click',()=>{if(!report)return;baseline={...structuredClone(report),context:$('result-context').textContent};renderComparison();$('status').textContent='Result kept in this tab. Choose another scenario or edit the inputs, then run again.';});
$('clear-comparison').addEventListener('click',()=>{baseline=null;renderComparison();});
$('save-readable').addEventListener('click',()=>{if(report)download(id+'-report.md',reportMarkdown(id,report.result,report.inputSource),'text/markdown;charset=utf-8');});
