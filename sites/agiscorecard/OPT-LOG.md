...
两周期台账结构:2025-08~09 用户被官方 postmortem 证实为对(三 bug,峰值 16% Sonnet 4
请求)+ 2026-08 开放周期(A/B 报告 vs 官方 display bug 说法)+ 四混淆分支(标注为
分析框架)+ 成文双向翻转条件。数字全部引 postmortem 原文。今日 agi 共 3 页,
在「预审 2-3 页/轮」额度内;下一轮回到单页节奏。
...
防翻炒规则下本轮不再新增内容。唯一优化：zh 审计页漏了判定型六件套第⑥条
（首屏活数字），补 62.5 胶囊 + index_click{zh_sun2013_live} 埋点。
监控要点：`site_search{mcp}` 出现 2 次经核实为扫描器金丝雀探针（同秒、label
自称 canary）——**不是首个 agent 调用里程碑**，按 CI/探针陷阱规则如实记录；
api.microsoft.ai 引荐 1 次 + EA 论坛引荐 2 次（首次出现该社区引荐源）；
`index_click{*_live}` 上线 7 天仍 0，判定日 8/30。订阅 2（均 stored）。
...
## 2026-08-31(每日 run，周一档)— 闸门全关，但赔率出现首次实测位移
- **spec(三轮)**：周一 = 日更 + 三项周一专属；自我批判：①昨夜 goldrush 已连推 5 次，今日必须
  合并单次 push ②我昨天刚给 goldrush 立了「09-30 前不许再写白皮书提案」，今天不得自我豁免。
- **ship(唯一新内容，且是数据触发的)**：`/agi-odds-vs-evidence` **Issue #2** —— 市场 Yes
  7.5%→**18.0%**（+10.5 点、2.4×，成交额 +7.6%，市场未关），同期 Tracker 62.5/100 与八条判定
  全部未动。本系列首次两端均机器核实带时间戳的对比（08-24 只记指示性、不发刊，正是因为基线是
  手抄的）。按该页自订纪律「有一侧真动了才发刊」执行。**限度照写**：单一薄市场、一周一个点、
  驱动原因不编造；回吐照样进复查表。
- **⓪+ 引用放大队列：两项均未过自身前置条件，不动工、不塞猜的选题**（英文 eu-ai-act 28 天 1 次
  浏览 vs TOP10 门槛 12；capex 引用趋势需 09-01~03 的 Bing 明细）。已写回 backlog。
- **防误报第 4 次**：`site_search{mcp}` 三条形状酷似「首个 agent MCP 调用」，过四条件即死
  （UA 全 bot、430ms 扫三个工具、落在本会话工作流窗口）。**不报里程碑。** 连带堵掉测量陷阱：
  自家 MCP 工具抓自家 `/claimledger.json`，已加入 fetchlog 排除项 c。
- **深审计**：09-30 判定线仪器体检——服务端 `subscribers`(2) 与 JS `sub_ok`(2) 精确吻合，
  仪器无故障，可按真实数据裁决（当前 2/5）。
- **四站记分板**：eco 离钱转化 12.5% vs agi 1.4% / bpj 1.2% / tds 0.5%；agi 判定日 11-15，本周不加码。
...
## 2026-09-12(每日 run)— 昨天的「+23% 增长」是一台每 6 分钟敲三个页面的探针

**spec(两轮自批后)**:⓪/⓪+ 闸门仍未过;①的两臂(sasummary 09-09、when-will 09-11)都在跑,
判定日 10-07,**不许提前加码**。所以今天本该是 ⑤。自批一:动 ⑤ 之前先把读数核一遍——
这两天已经有两次「数字对不上」。自批二:**两条都真的对不上,而且都得修**。
所调 skill:analytics。

### 一、昨天没有增长。231 里有 182 是探针。
第一眼读数:09-11 JS `page_view` **231**,前十天基线 49–70,看着 +3.5 倍,
28 天口径会写成「+23%」。**逐行拆开就散了**:
- `/amodei-white-collar-bloodbath-prediction` 31 · `/about` 30 · `/progress-index` 30,
  **三个固定路径**,全部 US,全部落在 **12:12–15:12 UTC** 三小时内 →
  **每条约每 6 分钟一次,三条完全同步**。
- 而且每一次同时以「内部 referrer」和「无 referrer」各记一条,所以是 **91×2 = 182**。
- UA 是一条普通 Windows Chrome,**按 UA 分不出来**;能分出来的只有行为。
- 扣掉后 09-11 真实约 **49**,与基线一致。**那天没有增长。**

这正是站规里「CI 自测会伪装成增长」的同一类,只是这次不是我们自己的 CI,
而且它骗过了本站一直当作金标准的 **JS 口径**——`events.page_view` 只证明 JS 跑了,
不证明那是人。已把行为特征与那条排查 SQL 写进 CLAUDE.md:
**任何单日 JS pv 跳升 ≥2 倍,先按 path×country×时段拆,再决定要不要写进日报。**
**刻意没做的事**:没有把这类 UA 加进 bot 正则——它长得就是普通浏览器,按 UA 封会误杀真读者,
站规原文说得很清楚,那个方向的错是静默抹掉真实读者。行为特征只用于读数时扣除,不用于分类。
**真能按 UA 判的那条顺手修了**:`panscient`(整条 UA 就是 `panscient.com`,09-11 敲 288 次
却被算成 human)已加进正则,并在注释里留了证据。

### 二、fleet heartbeat 给 agiscorecard 报了「6 天没部署」,而它当天部署了两次。
09-11 14:10 的快照写着 `days_since_deploy: 6`,但当天 04:07 与 14:09 各成功部署过一次。
用同样的查询串事后复查四种变体(`status=success` / 无过滤 / `status=completed` / 加 branch),
**全部返回正确的最新一条**;近 10 条成功记录也没有任何空档。也就是说**当时那次读取拿到了
过期结果**,根因无法事后确证(最可能是 API 读副本的短暂不一致)。

**查不出根因就不猜,改成对这类读取免疫**:
- 取最近 5 条成功 run 的 `updated_at` **最大值**,不再盲信 `[0]`;
- 把拿到的原始值打进日志(`probe <site>: http=.. last_success=..`),下次再出是可诊断的,
  不用靠推。**教训同源**:itch 那次也是靠「步骤耗时」推断而不是读日志,推错了。
- **自动重新部署改为必须连续两次读到**(与上一份快照比对,jq 读回)。这一步有外部副作用,
  凭一次假读数派发部署不只是浪费一次构建,更糟的是会把这个字段训练成「经常乱报、可以无视」,
  **而一个被无视的健康信号等于没有**。代价:真停摆时晚一天发现——可以接受。
- 本地把这段 bash 单独跑过:`age=8 & prev=6 → hold`、`age=8 & prev=8 → REDEPLOY`、
  `prev` 缺失 → hold,三条都对。顺带避掉一个坑:第一版把读上一份快照写成内联 python 多行串,
  **YAML 缩进会让它在 runner 上 IndentationError,而本地完全看不出来**,已改用 jq。

**validate OK(229 页 / 211 URL);sanitiser 26 条断言通过。**

## 2026-09-13(每日 run)— 给最陈旧的高流量对比页补上它缺的那一条,并确认 Metaculus 真的取不到

**spec(两轮自批后)**:⓪/⓪+ 闸门未过;①两臂(09-09 sasummary、09-11 when-will)都在跑,
判定日 10-07,**今天不许再加码**——多加一臂会让 10-07 那天分不清是位置起效还是铺得多。
所以落 ⑤。自批:⑤ 的目标 `/aschenbrenner-vs-metaculus`(06-30 起 75 天未动,陈旧页里
流量最高 9 pv)09-10 判过「核不到 Metaculus 现值就不能改」——但那次只试了 curl。
**今天换 WebFetch 再试一次:同样 `EGRESS_BLOCKED`。** 两条独立通道都确认,
这个数字在本沙箱里就是取不到,记进台账,后续会话别再重试。

**但「不能改那个数」不等于「这页没得做」。** 09-10 那次顺带发现:本页**缺 CLAUDE.md
判定型页清单的第 ⑥ 条**——首屏没有活数字。而这一页恰恰最需要它:它整页在比
「A 说 2027 / B 说 2033」,**两边都是嘴上的年份,谁都没有被评分**;本站唯一能加的
第三样东西,就是**这两个预测共同押的那个赌,今天兑现到哪了**。

**ship(一处,零编造)**
首屏加活数字区块,标题写成 "Neither forecast is graded. This is."——
62.5/100 + 「八条里还有 4 条未决」+ as-of 2026-09-06,**全部从 data.json 读出后断言比对**
(8 条、4 条 Open/Pending、分数、日期),一个数都没有手打。埋点
`index_click{metaculus_live}` / `{metaculus_live_link}`。
数值由 09-10 上线的 `sync_live_hooks.py` 统一同步,`validate.py` 有闸门,**不新增维护负债**。

**这次 bump 了日期,和前两次不同,理由要说清楚**:09-09 与 09-11 只是移动一个已有 CTA,
不是内容更新,所以刻意不 bump;今天是**新增一个首屏区块**,是真实的内容更新,
所以 `Last updated` / `dateModified` / sitemap lastmod 三处一起走到 2026-09-13。
**Metaculus 那两个数字一个字没动**,它原本就带着 "as of early 2026" 的限定,保持原样才诚实。

**按 2026-08-30 的裁决,这个钩子不以点击考核**——它服务的是引用差异化,不是转化。

**顺带确认(不是今天做的,是昨天的修生效了)**:fleet heartbeat 09-13 00:09 那份快照
agiscorecard 读回 `d0`(昨天误报 6)。09-12 也没有再出现 09-11 那台探针——当天 50 pv /
25 路径 / 14 国家 / 单路径最高 14,形态正常,**那是一次性的**。

**validate OK(229 页 / 211 URL);hreflang 闭合;活数字钩子七页一致。**

**顺手记一件本轮才看见的事(未动手)**:仓内已有 `data/fleet-bets.json` 舰队预登记赌注台账,
**42 条 open / 0 条已结算**,最近一条 `fleet-ai-access-0916` 三天后到期,其 lose 分支写着
