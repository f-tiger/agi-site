...
PAGES = ["ai-entry-level-jobs", "sanfang-agreement", "yingjie-status", "resources"]
...
def convert(slug):
    ...
    lines = ["# " + title, "", "> " + desc, "", "来源: https://firstjob.agiscorecard.com/" + slug + ("  · 更新 " + updated.group(1) if updated else ""), ""]
...
def main():
    parts = ["# 第一份工作 — 全文(判定页与官方入口,机器可读)", "", "> 1270 万人抢第一份工作:入门级岗位的判定、条款与清单。本文件由页面自动生成;页面是唯一真源。站点说明见 https://firstjob.agiscorecard.com/llms.txt", ""]
    for slug in PAGES:
