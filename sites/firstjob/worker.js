...
export default {
  async fetch(request, env, ctx) {
    ...
    if (p === "/api/selftest" && request.method === "GET") {
      ...
      try {
        await ensureSchema(env.EV);
        const r = await env.EV.prepare("SELECT COUNT(*) n FROM jev WHERE label = ? AND day >= date('now','-2 days')").bind(label).first();
    ...
    if (p === "/api/pulse" && request.method === "GET") {
      ...
      try {
        ...
        const q2 = await env.EV.prepare(
          "SELECT ref AS host, COUNT(*) AS n FROM jev WHERE name='page_view' AND ua_class='human' AND day >= date('now','-28 days') GROUP BY ref ORDER BY n DESC LIMIT 1000"
        ).all();
