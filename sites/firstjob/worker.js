// 第一份工作 worker:静态资产 + /e 白名单事件 + 服务端 page_view + ua_audit + /api/pulse。
...
async function ensureSchema(db) {
  ...
  await db.batch([
    db.prepare("CREATE TABLE IF NOT EXISTS jev (day TEXT, ts TEXT, name TEXT, label TEXT, value INTEGER, path TEXT, ref TEXT, ua_class TEXT, country TEXT)"),
    db.prepare("CREATE TABLE IF NOT EXISTS jua_audit (day TEXT, ua_prefix TEXT, ua_class TEXT, hits INTEGER, PRIMARY KEY (day, ua_prefix, ua_class))"),
  ]);
  schemaReady = true;
}
function uaClass(ua) {
...
function auditUa(env, ctx, ua, cls) {
  if (!env.EV) return;
  ctx.waitUntil((async () => { try { await ensureSchema(env.EV); await env.EV.prepare("INSERT INTO jua_audit (day, ua_prefix, ua_class, hits) VALUES (date('now'), ?, ?, 1) ON CONFLICT(day, ua_prefix, ua_class) DO UPDATE SET hits = hits + 1").bind((ua || "").slice(0, 48) || "(none)", cls).run(); } catch (e) {} })());
}
function logRow(env, ctx, row) {
  ...
  ctx.waitUntil((async () => {
    try {
      ...
      await env.EV.prepare("INSERT INTO jev (day, ts, name, label, value, path, ref, ua_class, country) VALUES (date('now'), datetime('now'), ?, ?, ?, ?, ?, ?, ?)")
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (p === "/api/selftest" && request.method === "GET") {
      ...
      try {
        await ensureSchema(env.EV);
        const r = await env.EV.prepare("SELECT COUNT(*) n FROM jev WHERE label = ? AND rowid > (SELECT MAX(rowid) FROM jev) - 500 AND day >= date('now','-2 days')").bind(label).first();
    ...
    if (p === "/api/pulse" && request.method === "GET") {
      ...
      return cachedJson(request, env, ctx, 3600, async () => {
        try {
          await ensureSchema(env.EV);
          const q = await env.EV.prepare("SELECT ref AS host, COUNT(*) AS n FROM jev WHERE name='page_view' AND ua_class='human' AND day >= date('now','-28 days') GROUP BY ref ORDER BY n DESC").all();
