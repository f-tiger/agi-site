...
export async function saveSpace(db,m,b){
 ...
 const data=b.data;if(!data||data.version!==1||typeof data.product!=='string'||!/^[a-z0-9-]{1,60}$/.test(data.product)||!data.values||typeof data.values!=='object'||Array.isArray(data.values))throw Error('bad_backup');
