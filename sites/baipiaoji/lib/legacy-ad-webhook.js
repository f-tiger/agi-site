...
export async function onRequestPost({ request, env }) {
  ...
  if (!['checkout.session.completed','checkout.session.async_payment_succeeded'].includes(ev.type)) return json({ ok: true, code: 'ignored' });
  ...
  const dupe = await env.HITS.prepare('SELECT ad_id FROM ad_orders WHERE event_id = ? OR session = ?')
    .bind(evId, session).first();
  if (dupe) return json({ ok: true, code: 'already' });

  // 延迟到账的支付方式(如 SEPA 直接借记)会在钱到之前就触发 completed。
  // 不判这一条,位子会在钱还没到的时候就上线。
  if (s.payment_status !== 'paid') return json({ ok: true, code: 'unpaid' });

  const row = await env.HITS.prepare(
    'SELECT id, cat, url, price_cents, currency, status FROM ads WHERE id = ?').bind(id).first();
  ...
  ).bind(today, expDay, amount, session, id).run();
