...
import {ensureMembers,memberSite} from '../../lib/membership.js';
...
export async function onRequestPost({request,env}){
 ...
 if(b?.action==='stats'){
  await ensureMembers(env.HITS,memberSite(env));
 ...
 if(b?.action==='support'){
  await ensureMembers(env.HITS,memberSite(env));return json({ok:true,requests:(await env.HITS.prepare('SELECT id,member_id,message,created FROM wb_support WHERE resolved=0 ORDER BY created LIMIT 20').all()).results});
 }
 if(b?.action==='resolve'&&typeof b.id==='string'){
  await ensureMembers(env.HITS,memberSite(env));await env.HITS.prepare('UPDATE wb_support SET resolved=1 WHERE id=?').bind(b.id).run();return json({ok:true});
 }
 if(!b||!['suspend','resume'].includes(b.action)||!/^[a-f0-9]{64}$/.test(b.order||''))return json({ok:false},400);
 try{await ensureMembers(env.HITS,memberSite(env));const r=await env.HITS.prepare('UPDATE wb_members SET suspended=? WHERE id=(SELECT member_id FROM wb_orders WHERE id=?)').bind(b.action==='suspend'?1:0,b.order).run();return json({ok:true,changed:r.meta?.changes||0});}catch{return json({ok:false},503);}
}
