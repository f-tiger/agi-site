...
export async function onRequestGet({request,env}) {
 ...
 if(u.searchParams.get('doctor')==='1'){
  let database=false;
  try{if(env.HITS){await ensure(env.HITS);database=true;}}catch{}
  const blockers=[];
  ...
  return json({ok:true,selling:cfg.ready&&database,mode:cfg.mode,
   rails:{card:cfg.ready&&database,wallet:false},
   ...
   hint:'配置齐全不代表已完成真实付款验收。新订单使用 Stripe Checkout；旧付款链接和钱包订单继续保留。'});
 }
