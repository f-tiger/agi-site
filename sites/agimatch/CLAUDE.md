...
- **需求门 —— ❌ 未通过。这是本站最诚实的处境，后续会话不许粉饰。**
  需求维度**被它自己的复核判为「未完成研究」**（不是「有保留地完成」）：零个一手页面被
  打开、没拿到任何查询的搜索量；连唯一被看好的 "AI automation consultant cost" 都被指出
  **既无法核实、又自我拆台**——它是查数字的问题，答案引擎零点击就解决了，在那儿拿引用
  只涨权威、不带线索。startup-radar 与 rising 面按定义是**选题输入不是需求证据**。
  供给侧反而查实了拥挤（155 个 AI 工具目录分布在 148 个主机；294 个自称
  "AI agent marketplace" 的仓库最高才 49 star）——**拥挤是需求信号的反面**。
  → **本站是在需求门未过的状态下上线的。唯一合法的补救是让预登记判定线到期照杀，
  不是继续加功能。加桶、加页面、加工具都补不了这一门。**
- **差异门 —— ✅ 通过，但轴线与站上原先宣称的不同**。原本宣称「价格有来源」，那半条
  已被打掉（见下）。真正立得住的是**对自身激励的可证伪承诺**：公开 routes.json +
  每条路由的 `affiliate` 布尔 + CI 门禁拒绝把付费链接挂在「不用雇人」的判定上。
  **这是聊天机器人结构上给不出的东西**，是本站唯一真护城河——**保护它优先于任何新功能**。
- **诚实门 —— 修补后通过**。当日发现并修掉三处自伤：虚假的联盟关系声明、被驳回的
  Gartner 厂商数字、以及下面这条价格来源过度声明。当前 10 桶里 6 桶给本站赚零收入，
  且页面上那个数由 JS 现算「有几条付费链接」。

### ⚠️ 价格来源的真实成色（**永远别再写 "sourced prices"**）
表里 28 条引用**只有 2 条是厂商自己的页面**，26 条是第三方对比文——其中若干由**被定价
产品的竞争对手**发布（claap.io 给 Fireflies 定价、alhena.ai 与 sitegpt.ai 给聊天机器人
定价、parseur.com 给 n8n/Zapier/Make 定价）。而页面原本在七处宣称「每条价格都有来源」。
**差异化本身就是可审计性，而审计链接指向利益相关方——这是最自伤的一种过度声明。**
...
3. **自带变现闭环** — ❌ **不满足**：路由点击→联盟在结构上是闭环，但**联盟账号尚未申请**，
   v1 上线时收入结构性为零。复核另加一条更硬的经济约束（见 owner 待办的 FTB 条）：
   Fiverr 只对**首次买家**付佣，而本站受众是技术上老练的 AI 买家，**大概率早就有账号**。

**⚠️ 综合裁决：1 条明确通过、1 条勉强通过、1 条不通过。按舰队铁律「差一条都不开」
（2026-08-23），这个子域本不该开。** 它是在 owner 2026-08-30 明确指示下开的——那是对
自设禁令的**合法覆盖**，但**不能追溯性地满足铁律**。因此正确姿态是:
**本子域处于观察期（on probation），对着日期线，不是已被证明成立。**
下一次有人想给它加功能之前，先回读这一段。
...
1. **联盟账号（变现的前提，v1 上线时全部未办，页面因此只放普通链接）**：
   - **⚠️ 以下全部是 snippet 级证据（域名限定搜索，片段只可能来自该计划自己的域名），
     没有一页被真正打开过（沙箱 egress 全封）。上公开页面之前必须人工回源核对一次；
     一律以 owner 实际后台为准。**
   - **Fiverr Affiliates**（`help-partnerships.fiverr.com` 域内片段）：
     Marketplace CPA = 首单买家(FTB)金额 **25%** + 其余订单 **10%** 分成 12 个月；
     **Fiverr Pro CPA = FTB 金额 70%** + 同样 10%/12 个月；Logo Maker 固定 $30；
     动态 CPA 计划**上限 $500**；**cookie 30 天**；**不要求有网站**即可申请，
     但申请要**人工审核**（时长未知 → **不要假设"第一周就能变现"**）。
     一方深链工具：后台 Marketing Tools → Default and Deep Links → LP URL。
   - **🔴 最重要的经济约束（会决定这条线的成败，别用点击量估收入）**：
     Fiverr **只对首次买家（FTB）付 CPA**——「pays out commissions only when new buyers…
     place their first-ever order」。**本站受众是技术上老练的 AI 买家，恰恰是最可能
     早就有 Fiverr 账号的人群**，所以有效转化率会远低于任何公开基准。
     11-30 判定线里「点了不成交」那一条，最可能的原因就是这个，不是页面做得不好。
   - **Fiverr AI 分类深链目标**（换联盟链接时用这些，不要指首页）：
     `/categories/ai-services`、`/categories/programming-tech/ai-coding`、
     `/categories/programming-tech/ai-coding/ai-integrations`、
     `/categories/business/ai-consulting`、`/discover-ai-agents`、
     **`pro.fiverr.com/cat/ai-expert`（Pro 付 70%，优先级最高）**。
   - **Upwork**（`upwork.com/affiliates` 域内片段）：新客户**首份合同支出的 70%**，
     **单笔上限 $150**。注意转化事件是**已注资的合同**（客户得发单、雇人、真花钱），
     不是注册——这是一条我们控制不了的多周漏斗。
     **cookie 时长 = UNKNOWN**：原先写的「30 天」经复核判定很可能是**从 Fiverr 串过来的**，
     没有任何来源确立过 Upwork 的窗口。已删，别再凭印象填回去。
     「走 Impact 平台」这条本身也是 UNVERIFIED。
   - **Freelancer.com / PeoplePerHour / Guru 一律别申请**：Freelancer 的 Bonus 是
     **会过期的站内额度**且要求与被推荐人有既存关系；PeoplePerHour 付**账户额度不是现金**；
     Guru 看不到有联盟计划。
