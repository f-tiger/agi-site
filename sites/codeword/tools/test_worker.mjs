// codeword worker 单测:假 D1 跑通 page_view 落表 cev / ci 排除 / 白名单 / pulse。
...
class FakeD1 { constructor(){ this.rows=[]; } prepare(sql){ const self=this; return { sql, args:[], bind(...a){ this.args=a; return this; }, async run(){ return self.exec(this.sql,this.args); }, async first(){ return self.exec(this.sql,this.args); }, async all(){ return { results: await self.exec(this.sql,this.args) }; } }; } async batch(s){ return s.map(()=>({})); }
  exec(sql,a){ if(/^CREATE/.test(sql)) return {}; if(sql.startsWith("INSERT INTO cev")) { this.rows.push(a); return {}; } if(sql.startsWith("INSERT INTO cua_audit")) return {}; if(sql.startsWith("SELECT '_total'")) return [{host:"_total", n: this.rows.filter(r=>r[0]==="page_view"&&r[5]==="human").length}]; if(sql.startsWith("SELECT COUNT(*) n FROM cev WHERE label")) return { n: this.rows.filter(r=>r[1]===a[0]).length }; if(sql.startsWith("SELECT ref AS host")) { const g={}; for(const r of this.rows) if(r[0]==="page_view"&&r[5]==="human"){ const k=r[4]||""; g[k]=(g[k]||0)+1; } return Object.entries(g).map(([host,n])=>({host,n})); } throw new Error("unhandled "+sql.slice(0,40)); } }
...
const call=(path,opts={})=>worker.fetch(new Request("https://codeword.agiscorecard.com"+path,{ ...opts, headers:{ "content-type":"application/json", "user-agent":"Mozilla/5.0 test", ...(opts.headers||{}) } }),env,ctx);
...
ok(r.status===200 && db.rows.some(x=>x[0]==="page_view"&&x[3]==="/ai-face-voice-scam"&&x[5]==="human"), "静态页透传 + page_view(human) 落 cev");
...
for (const ref of ["https://www.google.com/search?q=x","https://www.google.com/","https://chatgpt.com/",
                   "https://play.agiscorecard.com/","https://codeword.agiscorecard.com/details","https://netflix.com/"]) {
