...
export default {
  async fetch(request, env, ctx) {
    ...
    if (p === "/api/selftest" && request.method === "GET") {
      ...
      try {
        await ensureSchema(env.EV);
        const r = await env.EV.prepare("SELECT COUNT(*) n FROM cev WHERE label = ? AND rowid > (SELECT MAX(rowid) FROM cev) - 500 AND day >= date('now','-2 days')").bind(label).first();
    ...
    if (p === "/api/pulse" && request.method === "GET") {
      ...
      return cachedJson(request, env, ctx, 3600, async () => {
        try {
          await ensureSchema(env.EV);
          const q = await env.EV.prepare("SELECT ref AS host, COUNT(*) AS n FROM cev WHERE name='page_view' AND ua_class='human' AND day >= date('now','-28 days') GROUP BY ref ORDER BY n DESC").all();
