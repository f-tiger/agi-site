...
def match(radar, rising, covered, supply, today):
    ...
    for it in [*(((src.get("reddit_requests") or {}).get("items") or [])), *(((src.get("reddit_wish") or {}).get("items") or [])), *(((src.get("hn_ask") or {}).get("items") or [])), *(((src.get("softwarerecs") or {}).get("items") or [])), *(((src.get("bluesky_wish") or {}).get("items") or [])), *(((src.get("lemmy_wish") or {}).get("items") or []))]:
...
def main(argv):
    ...
    reddit_ok = {k: bool((src.get(k) or {}).get("ok")) for k in ("reddit_requests", "reddit_wish", "reddit_vertical", "hn_ask", "softwarerecs", "bluesky_wish", "lemmy_wish")}
    reddit_ok["reddit_access"] = radar.get("reddit_access")
