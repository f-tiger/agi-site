...
function storeKey(value){if(value!==key){selected=null;version=null;member=null;orders=[];$('payload').value='';$('name').value='';$('history').replaceChildren();$('space-list').replaceChildren(el('p',t.empty));$('member-status').textContent=t.inactive;renderOrders();}key=value;$('key').value=key;try{sessionStorage.setItem(keySlot,key);}catch{}}
...
$('key-file').onchange=async()=>{try{const f=$('key-file').files[0];if(!f||f.size>2048)throw Error(t.badKey);const j=JSON.parse(await f.text());if(j.service!==service||!/^([a-f0-9]{64})$/.test(j.key||''))throw Error(t.badKey);const draft=!selected?$('payload').value:'',name=!selected?$('name').value:'';storeKey(j.key);$('payload').value=draft;$('name').value=name;$('key-saved').checked=true;await refresh();}catch(e){notice(e.message,true);}};
