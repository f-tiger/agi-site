...
Each site runs an independent member portal, account store, payment endpoint and cloud workspace. Membership rights do not transfer across sites. `member-entry-build.mjs` now builds the full local portal; there is no BPJ handoff. See member-studio/README.md for the current scope.
